...
void Thread:: Print() 
{
        ...
        const char* TStoString[] = {"JUST_CREATED", "RUNNING", "READY", "BLOCKED"};  
     printf("tname:%s, tid: %d, uid: %d,pri: %d, tstatus: %s \n", this->getName(),this->getThreadId(),this->getUserId(),this->getPriority(),TStoString[this->getStatus()]); 
     }
...
//change this thread's status in allTread
...
void TS(){
    const char* TStoString[] = {"JUST_CREATED", "RUNNING", "READY", "BLOCKED"};
    printf("UID\tTID\tNAME\tPRI\tRUNT\tUSEDT\tSTATUS\n");
    for (int i = 0; i < MAX_THREAD_NUM; i++) { 
        if (tFlag[i]) {
            printf("%d\t%d\t%s\t%d\t%d\t%d\t%s\n", allThread[i]->getUserId(), allThread[i]->getThreadId(),allThread[i]->getName(), allThread[i]->getPriority(),allThread[i]->getRunTime(),allThread[i]->getUsedTime(), TStoString[allThread[i]->getStatus()]);
        }
    }
}
